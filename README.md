...
**WISEBot** is a *personal AI assistant* you run on your own devices.
...
# Send a message
moltbot message send --to +1234567890 --message "Hello from WISEBot"
...
WISEBot connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.
...
WISEBot can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
Notes:
- `gateway.bind` must stay `loopback` when Serve/Funnel is enabled (WISEBot enforces this).
...
WISEBot was built for **Molty**, a space lobster AI assistant. 🦉
